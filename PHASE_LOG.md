...
- 22.x Email provider config moved out of `.env` into a DB-backed admin UI (`/admin/email`, mirror of the AI-provider pattern from Phase 23.4). Migration `0021_email_provider_settings.sql` adds the singleton table; KMS-sealed secrets (SMTP password, Postmark token, EmailIt key) at rest; `.env` block stays as fallback for fresh installs. Send call sites (magic-link, scheduler-queue, schedules route, workbench email-PDF) switched from a boot-cached provider to per-request `resolveEmailProvider()` so admin updates take effect without restart. *(2026-05-07.)*
