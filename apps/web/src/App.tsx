...
const AdminEmailPage = lazy(() =>
  import("@/pages/AdminEmail").then((m) => ({ default: m.AdminEmailPage })),
);
    ...
    <ErrorBoundary>
      <ThemeProvider>
        <QueryClientProvider client={queryClient}>
          <AuthProvider>
            {/*
              basename: react-router needs to know the path prefix the
              SPA is served under so client-side navigation stays
              inside it. Vite's `base` build option populates
              import.meta.env.BASE_URL — "/" in standalone deploys (no-
              op) and "/<slug>/" behind the Vibe-Appliance shared Caddy
              in LAN / Tailscale modes (the entrypoint sed-replaces
              /__VIBE_BASE_PATH__/ in the built bundle). Without this,
              <Navigate to="/login" /> escapes the prefix and the
              browser ends up at /login instead of /<slug>/login.
            */}
            {/*
              Strip trailing slash before passing to BrowserRouter —
              React Router rejects basenames that end with `/`
              (docs: "path without trailing slash"). Vite's BASE_URL
              preserves the trailing slash from the `base` config
              (e.g. `/calculators/`), so a bare BASE_URL lands React
              Router at `basename="/calculators/"` → <Navigate to="/login"/>
              emits `/calculators//login` and routes fail to match
              under the Vibe-Appliance path-prefix mount.
            */}
